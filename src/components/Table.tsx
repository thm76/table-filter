...
import {createMemo, createSignal, For, Index, Show, splitProps,} from "solid-js";
...
export const Table: HopeComponent<"table", TableProps<any>> = (props) => {
    ...
    const highlight = createMemo<string|undefined>(() => props.filterMode === "simple" ? filters()[0]?.data : undefined)
                                        ...
                                        <Select
                                            ...
                                            onChange={(value) => {
                                                ...
                                                if (field) {
                                                    setFilters(($) => [
                                                        ...$.slice(0, index),
                                                        {...$[index], field, config: field.filterConfigs[0]},
                                                        ...$.slice(index + 1),
                                                    ]);
                                                }
                                            }}
                        ...
                        {(obj) => (
                            <Tr>
                                <For each={props.fields}>
                                    {(field) => (
                                        <Td>
                                            {field.component({obj, highlight: highlight()})}
                                        </Td>
                                    )}
                                </For>
                            </Tr>
